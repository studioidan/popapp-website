...
export default function Contact() {
  ...
  const inp:React.CSSProperties={width:'100%',background:'rgba(255,255,255,0.04)',
    ...
    fontFamily:'inherit',fontSize:'0.95rem',outline:'none',transition:'border-color 0.2s,box-shadow 0.2s'}
  ...
  return (
    <>
          ...
          <div>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:12}}>
              {[
                ...
                <a key={c.label} href={c.href} style={{display:'flex',alignItems:'center',gap:12,
                  textDecoration:'none',background:'rgba(255,255,255,0.03)',backdropFilter:'blur(12px)',
                  border:'1px solid rgba(255,255,255,0.07)',borderRadius:14,padding:'14px 18px',transition:'all 0.25s'}}
          ...
          <div style={{background:'rgba(8,14,31,0.7)',backdropFilter:'blur(24px)',
            ...
            {status==='ok'?(
              <div style={{textAlign:'center',padding:'40px 0'}}>
                ...
                <p style={{color:'var(--text-secondary)',lineHeight:1.7}}>תודה {form.name}!<br/>אחזור אליך תוך 24 שעות.</p>
              </div>
            ):(
              <form onSubmit={submit} style={{display:'flex',flexDirection:'column',gap:18}}>
                ...
                >{status==='sending'?'⏳ שולח...':'שלח ואחזור אליך תוך 24 שעות →'}</button>
              </form>
            )}
          </div>
        </div>
      ...
      <footer style={{position:'relative',zIndex:10,padding:'24px clamp(20px,6vw,100px)',
        ...
        alignItems:'center',color:'var(--text-muted)',fontSize:'0.78rem',flexWrap:'wrap',gap:10}}>
