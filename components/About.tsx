'use client'
import { useEffect, useRef } from 'react'

const HIGHLIGHTS = [
  { icon: '🚀', title: 'מהיר להבין', desc: 'פגישה אחת מספיקה. אני מבין את הצורך, מזהה את הפתרון ומתחיל לבנות.' },
  { icon: '🧠', title: 'Full-Stack אמיתי', desc: 'Mobile, Web, AI, IoT, Backend, Cloud — בכל שכבה, בכל טכנולוגיה.' },
  { icon: '😄', title: 'כיף לעבוד איתי', desc: 'שקוף, ישיר, נגיש. תמיד בתמונה. לקוחות חוזרים — כי הדרך לא פחות חשובה מהיעד.' },
  { icon: '🏥', title: 'Medical & Startups', desc: 'ניסיון ייחודי עם סטארטאפים רפואיים — רגולציה, CGM APIs, HL7/FHIR.' },
]

export default function About() {
  const refs = useRef<(HTMLElement | null)[]>([])

  useEffect(() => {
    const obs = new IntersectionObserver(entries => {
      entries.forEach(e => {
        if (e.isIntersecting) {
          const el = e.target as HTMLElement
          setTimeout(() => {
            el.style.opacity = '1'
            el.style.transform = 'translateY(0)'
          }, Number(el.dataset.i || 0) * 100)
          obs.unobserve(el)
        }
      })
    }, { threshold: 0.1 })
    refs.current.forEach(el => el && obs.observe(el))
    return () => obs.disconnect()
  }, [])

  const setRef = (i: number) => (el: HTMLElement | null) => { refs.current[i] = el }

  return (
    <section id="about" style={{
      position: 'relative', zIndex: 10,
      padding: 'clamp(80px,10vw,140px) clamp(24px,8vw,120px)',
      borderTop: '1px solid var(--border)',
    }}>
      {/* ambient */}
      <div style={{ position:'absolute', bottom:0, right:0,
        width:'min(500px,60vw)', height:'min(500px,60vw)', borderRadius:'50%',
        background:'radial-gradient(circle, rgba(255,107,53,0.04), transparent 70%)',
        pointerEvents:'none' }} />

      <div style={{ maxWidth: 900, margin: '0 auto' }}>
        {/* label */}
        <div ref={setRef(0)} data-i="0" style={{
          display: 'inline-flex', alignItems: 'center', gap: 10,
          fontSize: '0.68rem', fontWeight: 700, letterSpacing: '3px',
          textTransform: 'uppercase', color: 'var(--accent)', marginBottom: 24,
          opacity: 0, transform: 'translateY(24px)',
          transition: 'opacity 0.7s ease, transform 0.7s ease',
        }}>
          <span style={{ width: 24, height: 1.5, background: 'var(--accent)', display: 'block' }} />
          מי אני
        </div>

        {/* main text */}
        <h2 ref={setRef(1)} data-i="1" style={{
          fontSize: 'clamp(1.6rem, 4vw, 3rem)', fontWeight: 900,
          letterSpacing: '-1.5px', lineHeight: 1.1, marginBottom: 32,
          opacity: 0, transform: 'translateY(24px)',
          transition: 'opacity 0.7s ease 0.1s, transform 0.7s ease 0.1s',
        }}>
          מהנדס תוכנה עצמאי.<br />
          <span style={{
            background: 'linear-gradient(135deg,#00e5ff,#0077ff)',
            WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent',
          }}>שנגע בכמעט כל תחום אפשרי.</span>
        </h2>

        <p ref={setRef(2)} data-i="2" style={{
          fontSize: 'clamp(1rem,1.8vw,1.15rem)', fontWeight: 300,
          color: 'var(--text-secondary)', lineHeight: 1.9, marginBottom: 16,
          maxWidth: 680,
          opacity: 0, transform: 'translateY(24px)',
          transition: 'opacity 0.7s ease 0.15s, transform 0.7s ease 0.15s',
        }}>
          15 שנה. עשרות פרויקטים. מאפליקציות עם מאות אלפי משתמשים, דרך מערכות IoT שמצילות חיים, ועד פלטפורמות AI רפואיות שעוברות ניסויים קליניים.
        </p>

        <p ref={setRef(3)} data-i="3" style={{
          fontSize: 'clamp(1rem,1.8vw,1.15rem)', fontWeight: 300,
          color: 'var(--text-secondary)', lineHeight: 1.9, marginBottom: 56,
          maxWidth: 680,
          opacity: 0, transform: 'translateY(24px)',
          transition: 'opacity 0.7s ease 0.2s, transform 0.7s ease 0.2s',
        }}>
          אני לא עובד עבורך — אני עובד <strong style={{ color:'var(--text-primary)', fontWeight:600 }}>איתך</strong>. פגישה אחת מספיקה כדי שאבין בדיוק מה צריך, ומשם — קוד שרץ תוך ימים, לא חודשים. בלי ישיבות מיותרות, בלי תקציב שנשרף.
        </p>

        {/* 4 highlights */}
        <div style={{
          display: 'grid',
          gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%,200px),1fr))',
          gap: 16,
        }}>
          {HIGHLIGHTS.map((h, i) => (
            <div key={h.title} ref={setRef(4 + i)} data-i={4 + i} style={{
              background: 'rgba(255,255,255,0.03)', backdropFilter: 'blur(12px)',
              border: '1px solid rgba(255,255,255,0.07)', borderRadius: 18,
              padding: 'clamp(20px,3vw,28px)',
              opacity: 0, transform: 'translateY(24px)',
              transition: `opacity 0.7s ease ${(4+i)*0.08}s, transform 0.7s ease ${(4+i)*0.08}s`,
            }}
              onMouseEnter={e => { e.currentTarget.style.borderColor='rgba(0,229,255,0.25)'; e.currentTarget.style.transform='translateY(-4px)' }}
              onMouseLeave={e => { e.currentTarget.style.borderColor='rgba(255,255,255,0.07)'; e.currentTarget.style.transform='' }}
            >
              <div style={{ fontSize: '1.8rem', marginBottom: 12 }}>{h.icon}</div>
              <div style={{ fontWeight: 700, fontSize: '0.98rem', marginBottom: 8, letterSpacing: '-0.3px' }}>{h.title}</div>
              <p style={{ color: 'var(--text-secondary)', fontSize: '0.84rem', lineHeight: 1.72 }}>{h.desc}</p>
            </div>
          ))}
        </div>

        {/* stats */}
        <div ref={setRef(8)} data-i="8" style={{
          display: 'flex', gap: 'clamp(24px,5vw,64px)', flexWrap: 'wrap',
          paddingTop: 48, marginTop: 48, borderTop: '1px solid var(--border)',
          opacity: 0, transform: 'translateY(24px)',
          transition: 'opacity 0.7s ease 0.5s, transform 0.7s ease 0.5s',
        }}>
          {[
            { num:'15+', label:'שנות ניסיון' },
            { num:'100K+', label:'משתמשים באפליקציה אחת' },
            { num:'50+', label:'פרויקטים שסיימתי' },
            { num:'4', label:'תחומי התמחות' },
          ].map(s => (
            <div key={s.label}>
              <div style={{
                fontSize: 'clamp(1.6rem,3vw,2.6rem)', fontWeight: 900, lineHeight: 1, marginBottom: 6,
                background: 'linear-gradient(135deg,var(--accent),#0077ff)',
                WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent',
              }}>{s.num}</div>
              <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>{s.label}</div>
            </div>
          ))}
        </div>
      </div>
    </section>
  )
}
